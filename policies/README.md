...
| Runtime, alternative | Apple's native `container` (Containerization framework) | Investigated, not adopted. Structurally interesting: each container gets its own lightweight VM via `Virtualization.framework` (no shared kernel across containers at all — Firecracker/microVM-style), which would sidestep the "shared kernel escape" concern more fundamentally than gVisor does. Checked on this Mac: arm64 (required, already satisfied), macOS 15.3.1, tool not installed. Confirmed against Apple's own repo (github.com/apple/container): it requires **macOS 26** — a major version jump from what's installed here, not a point update — and Apple's maintainers state they typically won't support issues on anything older. Not usable here until that upgrade happens, and not a decision to make lightly just for this. Even once available, it isn't a drop-in swap: it has its own CLI/API rather than `docker compose` compatibility, so adopting it means rewriting the whole `compose/` templating layer and re-verifying the egress-gateway network model (internal-only network + sidecar proxy) actually has an equivalent in its networking story. Worth revisiting once it matures and this Mac is on a supported macOS version — not before. |
